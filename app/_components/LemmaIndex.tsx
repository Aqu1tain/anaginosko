...
export default async function LemmaIndex() {
  ...
  return (
    <section className="mt-10 border-t border-base-300 pt-6 pb-6">
      ...
      <p className="mt-1 text-sm text-base-content/70">
        Les {lemmas.length.toLocaleString("fr-FR")} lemmes du Nouveau Testament, chacun avec sa
            ...
            <div className="collapse-content">
              <div className="flex flex-wrap gap-x-3 gap-y-1.5 pb-2">
                {groups.get(L)!.map((l) => (
                  <Link
                    key={l}
                    href={`/concordance/${encodeURIComponent(l)}`}
