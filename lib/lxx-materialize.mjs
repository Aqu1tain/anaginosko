...
// Texte servi d'une entrée d'arbitrage. Une entrée « maison » (traduction propre,
// ex. suscription de psaume que Giguet omet) porte un texte LIBRE dans `maison` :
// il est servi tel quel (il ne pointe aucun mot Giguet, donc `sources` = []). Sinon
// on matérialise les sources Giguet. Accepte aussi un simple tableau de sources
// (rétro-compat). Un seul instrument : runtime ET matérialiseur offline passent ici.
export function materializeEntry(gbook, entry) {
  if (entry && typeof entry === "object" && !Array.isArray(entry) && typeof entry.maison === "string" && entry.maison.trim())
    return entry.maison.trim();
  const sources = Array.isArray(entry) ? entry : entry?.sources ?? [];
  return materializeSources(gbook, sources);
}
